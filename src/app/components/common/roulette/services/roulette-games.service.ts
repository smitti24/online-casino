...
export class RouletteGamesService {
  public rouletteGames: Array<GamesVM> = new Array<GamesVM>();
  ...
  constructor(private _gamesService: GameService, private _jackpotService: JackpotService, private _loaderService: LoaderService){
    ...
    this.getAllRouletteGames();
  }

  public getAllRouletteGames(): void {
    this._gamesService.allGames
    .subscribe((games: Game[]) => {
      games.forEach(game => {

        if (game){
          if (this._gamesService.findCategory(game.categories, 'roulette')){
            const jackpot = this._jackpotService.allJackpots.find(e => game.id === e.game);

            this.rouletteGames.push({
              id: game.id,
              name: game.name,
  ...
  public updateJackpotAmount(): void {
    this._jackpotService.updateJackpotAmounts(this.rouletteGames);
  }
}
